...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    initializeAuth() {
      // Restore auth state from localStorage (JWT token only)
      if (typeof window !== 'undefined') {
        const token = localStorage.getItem('auth_token')
        
        if (token && token !== 'null' && token !== '') {
          try {
            // JWTトークンが有効な場合、Cognitoから最新のユーザー情報を取得する必要がある
            // ここでは既存のユーザー情報があれば一時的に復元し、
            // バックグラウンドでCognitoから最新情報を取得することも可能
            ...
            if (userInfoStr) {
              ...
              console.log('Feature A - Auth state restored from localStorage')
              console.log('- User:', userInfo.name)
              console.log('- Token exists:', !!token)
            } else {
              // ユーザー情報がない場合、JWTトークンのみ保持
              this.token = token
              console.log('Feature A - JWT token restored, but no user info found')
            }
          } catch (error) {
          ...
          console.log('Feature A - No valid JWT token found in localStorage')
        }
      }
    },
